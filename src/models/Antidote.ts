...
import AntidoteInterface from '../interfaces/Antidote';
import ModifiersInterface from '../interfaces/Modifiers';

export class Antidote extends Potion implements AntidoteInterface {
  constructor(name: string, type: string, modifiers: ModifiersInterface) {
    super(name, type, modifiers);
  }

  static create(ingredients: object, diseases: object): Antidote | null {
    ...
    for (let i = 0; i < diseasesArray.length; i++) {
      ...
      for (let j = 0; j < ingredientsArray.length; j++) {
        ...
        for (let k = 0; k < ingredient.effects.length; k++) {
          ...
          if (!isAntidoteEffect || !antidoteEffects.includes(ingredient.effects[k])) {
            match = false;
            break;
          }
  ...
  static getRestoredValue(prefix: string, attribute: string): number {
    let min: number;
    let max: number;
